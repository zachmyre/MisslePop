...
function spawnEnemies() { // Spawns enemies every second (1000 ms)
    setInterval(() => {
        ...
        // const color = `hsl(${Math.random() * 360}, 100%, 75%)` // changes color to random
        const colors = ['red', 'pink', 'blue', 'green', 'orange', 'yellow', 'cyan']
        const color = colors[Math.floor(Math.random()*colors.length)]
        //changes position of enemies spawn points
        const angle = Math.atan2(canvas.height / 2 - y, 
            canvas.width / 2 - x)
        const velocity = { //creates velocity object
            x: Math.floor(Math.random() * 0.1) + 0.1 ,
            y: 1 // speed of enemies falling
        }
        enemies.push(new Enemy(Math.random() * canvas.width, 0, radius, color, velocity))
    }, 1000)
} 


// ////////////////Create Missle Instance///////////////////
// ////////////////Create Missle Instance///////////////////
// ////////////////Create Missle Instance///////////////////
...
function animate() {
    ...
    if(score > 1000){
        c.fillStyle = 'rgba(220, 220, 220, 0.1)'
    }
    if(score > 5000) {
